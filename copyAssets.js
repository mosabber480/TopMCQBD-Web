...
async function syncMongoDbToCloudflare() {
  try {
    ...
    const HomeConfig = (await import('./src/models/HomeConfig.js')).default;
    const AdminSidebarConfig = (await import('./src/models/AdminSidebarConfig.js')).default;
    const PolicyConfig = (await import('./src/models/PolicyConfig.js')).default;
    const Question = (await import('./src/models/Question.js')).default;
    const User = (await import('./src/models/User.js')).default;

    const layout = await LayoutConfig.findOne().lean();
    const home = await HomeConfig.findOne().lean();
    const sidebar = await AdminSidebarConfig.findOne().lean();
    const policy = await PolicyConfig.findOne().lean();
    ...
    const liveConfigsJs = `// Auto-generated by copyAssets.js from live MongoDB
...
export const initialPolicy = ${JSON.stringify(policy?.content || '', null, 2)};
export const initialQuestions = ${JSON.stringify(questions || [], null, 2)};
export const initialUsers = ${JSON.stringify(users || [], null, 2)};
`;
