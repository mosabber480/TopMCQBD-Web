...
const CACHE_KEY = 'topmcqbd_dbquestionbank_admin_cache';

export default function DBQuestionBankAdminPage() {
  ...
  const getApiUrl = useCallback(() => {
    ...
    return process.env.NEXT_PUBLIC_QUESTION_BANK_API_URL || 'https://question-bank-paid-api.onrender.com';
  }, []);
  ...
  const fetchQuestionBankData = useCallback(async () => {
    ...
    try {
      ...
      const endpoint = `${baseUrl}/api/db-test/question-bank`;
      ...
      setFetchError(
        `${err.message || 'Failed to connect'}. (নিশ্চিত করুন সার্ভার চালু আছে ও .env ফাইলে MONGODB_URI_QUESTION_BANK সঠিক রয়েছে।)`
      );
    } finally {
      setLoading(false);
    }
  ...
  const saveAllNewData = async (e) => {
    e.preventDefault();
    const validRows = newDataRows.filter((r) => r.text && r.text.trim() !== '');
    if (validRows.length === 0) {
      showTopAlert('অনুগ্রহ করে অন্তত একটি ফিল্ডে ডাটা বা টেক্সট লিখুন।', 'warning');
      return;
    }
    ...
    try {
      ...
      showTopAlert(
        `${successCount} টি ডাটা সফলভাবে db-question-bank-test কালেকশনে যুক্ত হয়েছে!`,
        'success'
      );
      setIsAddingData(false);
      setNewDataRows([{ text: '' }]);
      try { localStorage.removeItem('topmcqbd_dbquestionbank_test_cache'); } catch (e) {}
      await fetchQuestionBankData();
    } catch (err) {
      showTopAlert(`ডাটা যুক্ত করতে সমস্যা হয়েছে: ${err.message}`, 'danger');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDeleteText = async (id) => {
    ...
    try {
      const baseUrl = getApiUrl();
      const endpoint = `${baseUrl}/api/db-test/question-bank?id=${id}`;
      const res = await fetch(endpoint, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
      });
  ...
  return (
    <DbAuthGuard activeRoute="/db-connection/dbquestionbank-admin">
      <main className="db-page-container">
        <div className="db-content-card">
          {/* Header */}
          <div className="db-header">
            <div className="db-badge">
              Question Bank Admin Diagnostic & Manager
            </div>
            <h1 className="db-title">Question Bank Database Manager</h1>
            <p className="db-subtitle">
              Question Bank Cluster (<strong>TopMCQBD_DB_Question_Bank</strong>) ও <code>db-question-bank-test</code> কালেকশন ম্যানেজমেন্ট ও লাইভ ডায়াগনস্টিক
            </p>
          ...
          <div className="db-control-bar">
            ...
            <div className="control-btn-group">
              <button onClick={fetchQuestionBankData} disabled={loading} className="recheck-btn">
                {loading ? (
                  <>
          ...
          {fetchError && (
            <div className="alert-card alert-error">
              ...
              <small className="alert-hint">
                * নিশ্চিত করুন যে সার্ভার সচল আছে এবং <code>.env</code> ফাইলে সঠিক <code>MONGODB_URI_QUESTION_BANK</code> দেওয়া আছে।
              </small>
            </div>
          )}
          ...
          <div className="db-grid" style={{ gridTemplateColumns: '1fr', marginBottom: '24px' }}>
            <div className={`status-card ${statusData?.connected ? 'card-success' : 'card-danger'}`}>
              <div className="card-header">
                <div>
                  <div className="card-type-tag">Question Bank Paid Cluster</div>
                  <div style={{ fontSize: '12px', color: '#94a3b8', marginTop: '3px' }}>
                    টার্গেট কালেকশন: <strong>db-question-bank-test</strong>
                  </div>
                </div>
              ...
              <h3 className="card-db-name">
                📁 {statusData?.cluster || 'TopMCQBD_DB_Question_Bank'}
              </h3>
              ...
              <div className="meta-list">
                ...
                <div className="meta-row">
                  <span className="meta-label">db-question-bank-test কালেকশনে মোট ডাটা:</span>
          ...
          <div className="crud-container">
            <div className="crud-header">
              <h3>
                ...
                ডাটা যোগ ও সম্পাদনা প্যানেল (db-question-bank-test)
              </h3>
              <p>নিচের বক্সে টেক্সট লিখে যোগ করুন। এটি সরাসরি Question Bank MongoDB ক্লাস্টারে সেভ হবে এবং <code>/db-connection/dbquestionbank-test</code> পেজে লাইভ আপডেট হবে।</p>
            </div>
            ...
            <div className="summary-grid">
              ...
              <div className="summary-card">
                ...
                <strong className="summary-val" style={{ color: '#0284c7' }}>db-question-bank-test</strong>
              </div>
              <div className="summary-card">
          ...
          <DbNavBox activeRoute="/db-connection/dbquestionbank-admin" />
        ...
        <style jsx>{`
          ...
          :global(.right-link) {
            background: #008fb0;
            color: #ffffff;
            box-shadow: 0 2px 8px rgba(0, 143, 176, 0.25);
          }
          :global(.right-link:hover) {
