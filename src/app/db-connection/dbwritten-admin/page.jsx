...
const CACHE_KEY = 'topmcqbd_dbwritten_admin_cache';

export default function DBWrittenAdminPage() {
  ...
  const fetchWrittenData = useCallback(async () => {
    setLoading(true);
    setFetchError(null);
    try {
      const baseUrl = getApiUrl();
      const endpoint = `${baseUrl}/api/db-test/written`;
      ...
      setFetchError(
        `${err.message || 'Failed to connect'}. (নিশ্চিত করুন সার্ভার চালু আছে ও .env ফাইলে MONGODB_URI_WRITTEN সঠিক রয়েছে।)`
      );
    } finally {
      setLoading(false);
    }
  ...
  const saveAllNewData = async () => {
    ...
    try {
      ...
      for (const row of validRows) {
        const res = await fetch(`${baseUrl}/api/db-test/written`, {
          method: 'POST',
      ...
      setIsAddingData(false);
      setNewDataRows([{ text: '' }]);
      try { localStorage.removeItem('topmcqbd_dbwritten_test_cache'); } catch (e) {}
      setActionMsg({ type: 'success', text: `✅ ${validRows.length} টি ডাটা সফলভাবে "db-written-test" কালেকশনে যুক্ত হয়েছে!` });
      fetchWrittenData();
    } catch (err) {
  ...
  const handleDeleteText = async (id) => {
    ...
    try {
      const baseUrl = getApiUrl();
      const res = await fetch(`${baseUrl}/api/db-test/written?id=${id}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
      });
  ...
  return (
    <DbAuthGuard activeRoute="/db-connection/dbwritten-admin">
        ...
        <div className="db-content-card">
          ...
          <div className="db-header">
            ...
            <p className="db-subtitle">
              Written Exam Cluster (<strong>TopMCQBD_DB_written</strong>) ও <code>db-written-test</code> কালেকশন ম্যানেজমেন্ট ও লাইভ ডায়াগনস্টিক
            </p>
          ...
          <div className="db-control-bar">
            ...
            <div className="control-btn-group">
              <button onClick={fetchWrittenData} disabled={loading} className="recheck-btn" style={{ background: '#e11d48' }}>
                {loading ? (
                  <>
          ...
          {fetchError && (
            <div className="alert-card alert-error">
              ...
              <small className="alert-hint">
                * নিশ্চিত করুন যে সার্ভার সচল আছে এবং <code>.env</code> ফাইলে সঠিক <code>MONGODB_URI_WRITTEN</code> দেওয়া আছে।
              </small>
            </div>
          )}
          ...
          <div className="db-grid" style={{ gridTemplateColumns: '1fr', marginBottom: '24px' }}>
            <div className={`status-card ${statusData?.connected ? 'card-success' : 'card-danger'}`}>
              <div className="card-header">
                <div>
                  <div className="card-type-tag" style={{ color: '#e11d48' }}>Written Exam Paid Cluster</div>
                  <div style={{ fontSize: '12px', color: '#94a3b8', marginTop: '3px' }}>
                    টার্গেট কালেকশন: <strong style={{ color: '#f43f5e' }}>db-written-test</strong>
                  </div>
                </div>
              ...
              <h3 className="card-db-name">
                <i className="fa-solid fa-folder" style={{ marginRight: '8px', color: '#f43f5e' }} />
              ...
              <div className="meta-list">
                ...
                <div className="meta-row">
                  ...
                  <span className="meta-value" style={{ color: '#e11d48', fontSize: '15px' }}>
          ...
          <div className="crud-container">
            <div className="crud-header">
              <h3>
                <i className="fa-solid fa-pen-nib" style={{ marginRight: '8px', color: '#e11d48' }} />
            ...
            <div className="summary-grid">
              <div className="summary-card">
                ...
                <strong className="summary-val" style={{ color: '#e11d48' }}>
                  {statusData?.cluster || 'TopMCQBD_DB_written'}
                </strong>
              </div>
              <div className="summary-card">
              ...
              {!isAddingData ? (
                ...
                <div className="add-rows-container">
                  <div className="add-rows-header">
                    <span className="add-rows-title" style={{ color: '#e11d48' }}>
                      <i className="fa-solid fa-layer-group" style={{ marginRight: '8px', color: '#e11d48' }} />
                      নতুন ডাটা যোগ করুন:
                    </span>
                  </div>
                  ...
                  <div className="add-rows-list">
                    {newDataRows.map((row, rIdx) => (
                      <div key={rIdx} className="add-row-item">
                        <span className="row-num-badge" style={{ background: '#fff1f2', color: '#e11d48' }}>#{rIdx + 1}</span>
                        <input
                          type="text"
          ...
          <DbNavBox activeRoute="/db-connection/dbwritten-admin" />
