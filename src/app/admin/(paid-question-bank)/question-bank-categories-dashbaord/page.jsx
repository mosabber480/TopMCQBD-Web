...
function QuestionBankCategoriesDashboardContent() {
  ...
  const fetchConfig = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/question-bank/config', { cache: 'no-store' });
      ...
      console.error('Failed to load question bank config:', err);
      showTopAlert('প্রশ্নব্যাংক কনফিগ লোড করতে ব্যর্থ হয়েছে!', 'danger');
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveConfig = async (newCats = categories, newItems = items) => {
    ...
    try {
      ...
      const res = await fetch('/api/question-bank/config', {
        method: 'POST',
        headers: {
      ...
      const data = await res.json();
      if (data.success) {
        showTopAlert('প্রশ্নব্যাংক কনফিগারেশন সফলভাবে সেভ করা হয়েছে!', 'success');
  ...
  const handleOpenAddItem = () => {
    ...
    setItemForm({
      id: `qb-${defaultCat.toLowerCase()}-${Date.now().toString().slice(-4)}`,
      year: '',
      category: defaultCat,
      date: '২০২৪',
      ...
      status: 'সম্পূর্ণ সমাধানসহ উপলব্ধ',
      tags: `${defaultCat}, Preliminary, 2024`,
  ...
  const handleSaveItem = async (e) => {
    ...
    if (!itemForm.year.trim()) {
      showTopAlert('প্রশ্নব্যাংকের শিরোনাম/সাল আবশ্যক!', 'warning');
      return;
    }
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f1f5f9', paddingBottom: '80px' }}>
      ...
      <div style={{
        ...
        <div style={{ maxWidth: '1350px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '14px' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <h1 style={{ fontSize: '1.4rem', fontWeight: 800, color: '#0f172a', margin: 0 }}>
                প্রশ্নব্যাংক ক্যাটাগরি ও প্রশ্ন সেট ড্যাশবোর্ড
              </h1>
            </div>
            <p style={{ fontSize: '0.85rem', color: '#64748b', margin: '4px 0 0 0' }}>
              এখানে সংরক্ষিত প্রশ্ন সেট ও ক্যাটাগরি সরাসরি <span style={{ color: '#7c3aed', fontWeight: 600 }}>/question-bank</span> পেজে প্রতিফলিত হবে।
            </p>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
            ...
            <Link
              href="/admin/question-bank-questions-dashbaord"
              style={{
      ...
      <div style={{ maxWidth: '1350px', margin: '24px auto 0', padding: '0 20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: '1 1 320px', justifyContent: 'flex-end' }}>
            <div style={{ position: 'relative', width: '100%', maxWidth: '280px' }}>
              <input
                ...
                placeholder="প্রশ্নব্যাংক খুঁজুন (যেমন: 46th, BCS)..."
                style={{
                  width: '100%',
        ...
        {filteredData.length === 0 ? (
          <div style={{
            ...
            <p style={{ margin: 0, fontSize: '0.88rem' }}>সার্চ ফিল্টার পরিবর্তন করুন অথবা &quot;নতুন প্রশ্ন সেট যোগ করুন&quot; বাটনে ক্লিক করুন।</p>
          </div>
        ) : (
            ...
            {filteredData.map(item => {
              ...
              return (
                ...
                >
                  <div>
                    ...
                    {item.subjectStats && (
                      <div style={{
                        ...
                        <i className="fa-solid fa-layer-group" style={{ marginRight: '6px', color: '#9333ea' }}></i>
                        {item.subjectStats}
                      </div>
                    )}
                  ...
                  <div style={{ marginTop: '10px', paddingTop: '12px', borderTop: '1px solid #f1f5f9', display: 'flex', gap: '8px' }}>
                    <Link
                      href={`/admin/question-bank-questions-dashbaord?category=${encodeURIComponent(catSlug)}`}
                      style={{
                        flex: 1,
                    ...
                    >
                      ...
                      <span>প্রশ্নাবলি পরিচালনা</span>
                    </Link>

                    <Link
                      href={`/question-bank-questions?category=${encodeURIComponent(catSlug)}`}
                      target="_blank"
                      style={{
                        backgroundColor: '#f8fafc',
                        color: '#475569',
      ...
      {/* ------------------------------------------------------------- */}
      {/* QUESTION BANK ITEM MODAL (CREATE / EDIT) */}
      {/* ------------------------------------------------------------- */}
      {showItemModal && (
        <div style={{
          position: 'fixed',
          ...
          <div style={{
            ...
            <form onSubmit={handleSaveItem} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, color: '#334155', marginBottom: '4px' }}>
                  বিষয়ভিত্তিক প্রশ্নের বিন্যাস (Subject Breakdown)
                </label>
                <input
                  type="text"
                  value={itemForm.subjectStats}
                  onChange={(e) => setItemForm({ ...itemForm, subjectStats: e.target.value })}
                  placeholder="যেমন: বাংলা ৩৫, ইংরেজি ৩৫, গণিত ১৫, বিজ্ঞান ১৫..."
                  style={{ width: '100%', padding: '8px 12px', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '0.88rem' }}
                />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, color: '#334155', marginBottom: '4px' }}>
                    ডিসপ্লে ট্যাগ (যেমন: 47th বা Step-1)
                  </label>
                  <input
                    type="text"
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="BCS, 47th, Preliminary, 2024"
                  style={{ width: '100%', padding: '8px 12px', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '0.88rem' }}
                />
              </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div>
                <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, color: '#334155', marginBottom: '4px' }}>
                  ক্যাটাগরি আইডি (ইংরেজি, যেমন: Judicial)
                </label>
                <input
                  type="text"
                  value={newCatId}
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, color: '#334155', marginBottom: '4px' }}>
                  ক্যাটাগরি লেবেল (বাংলায়, যেমন: জুডিশিয়ারি)
                </label>
                <input
                  type="text"
                  value={newCatLabel}
...
export default function QuestionBankCategoriesDashboardPage() {
  return (
    <Suspense fallback={<div style={{ padding: '40px', textAlign: 'center' }}>লোড হচ্ছে...</div>}>
      <QuestionBankCategoriesDashboardContent />
    </Suspense>
  );
}
