    ...
    if (examId && subjectId && chapterId) {
      ...
      if (ch) {
        ...
        if (chapterId !== expectedSlug && decoded !== expectedSlug) {
          router.replace(`/subjective-all-mcqs-Practice-success?examId=${examId}&subject=${subjectId}&chapterId=${encodeURIComponent(expectedSlug)}`);
        }
      }
    }
        ...
        <div style={{ backgroundColor: '#ffffff', borderBottom: '1px solid #e2e8f0', padding: '12px 20px' }}>
          <div className="container" style={{ maxWidth: '1300px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
            <div style={{ fontSize: '0.9rem', color: '#64748b' }}>
              <Link href="/subjective-all-mcqs-Practice-success" style={{ color: '#0284c7', textDecoration: 'none', fontWeight: 600 }}>বিষয়ভিত্তিক মডেল টেস্ট</Link>
              <span style={{ margin: '0 8px', color: '#cbd5e1' }}>/</span>
              <Link href={`/subjective-all-mcqs-Practice-success?examId=${selectedExam.id}`} style={{ color: '#0284c7', textDecoration: 'none', fontWeight: 600 }}>{selectedExam.title}</Link>
              <span style={{ margin: '0 8px', color: '#cbd5e1' }}>/</span>
        ...
        <div className="container" style={{ maxWidth: '1300px', margin: '35px auto 0', padding: '0 20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(310px, 1fr))', gap: '16px' }}>
            {topicsList.map((top, idx) => {
              ...
              const qUrl = `/subjective-all-mcqs-Practice-questions?category=${encodeURIComponent(catSlug)}`;
