        ...
        <div className="container" style={{ maxWidth: '1300px', margin: '0 auto', padding: '0 20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '10px' }}>
            <div style={{ fontSize: '0.9rem', color: '#64748b' }}>
              <Link href="/subjective-all-mcqs-Practice-success" style={{ color: '#0284c7', textDecoration: 'none', fontWeight: 600 }}>MCQ</Link>
              <span style={{ margin: '0 8px', color: '#cbd5e1' }}>/</span>
              <Link href={`/subjective-all-mcqs-Practice-success?examId=${selectedExam.id}&subject=${selectedSubject.id}`} style={{ color: '#0284c7', textDecoration: 'none', fontWeight: 600 }}>{selectedSubject.name}</Link>
              <span style={{ margin: '0 8px', color: '#cbd5e1' }}>/</span>
              <span style={{ color: '#0f172a', fontWeight: 700 }}>অধ্যায় {chapterId}</span>
            </div>

            <button
              onClick={() => router.push(`/subjective-all-mcqs-Practice-success?examId=${selectedExam.id}&subject=${selectedSubject.id}`)}
              style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(310px, 1fr))', gap: '16px' }}>
            {topicsList.map((top, idx) => (
              <div
                key={idx}
                onClick={() => router.push(`/subjective-all-mcqs-Practice-questions?category=${encodeURIComponent(`${selectedExam.categoryName} > ${selectedSubject.name} > ${currentChapter.title} > ${top}`)}`)}
                style={{
                  backgroundColor: '#ffffff',
                  borderRadius: '12px',
                  padding: '16px 18px',
        ...
        <div style={{ backgroundColor: '#ffffff', borderBottom: '1px solid #e2e8f0', padding: '12px 20px' }}>
          <div className="container" style={{ maxWidth: '1300px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
            <div style={{ fontSize: '0.9rem', color: '#64748b' }}>
              <Link href="/subjective-all-mcqs-Practice-success" style={{ color: '#0284c7', textDecoration: 'none', fontWeight: 600 }}>MCQ</Link>
              <span style={{ margin: '0 8px', color: '#cbd5e1' }}>/</span>
              <Link href={`/subjective-all-mcqs-Practice-success?examId=${selectedExam.id}`} style={{ color: '#0284c7', textDecoration: 'none', fontWeight: 600 }}>{selectedExam.categoryName}</Link>
              <span style={{ margin: '0 8px', color: '#cbd5e1' }}>/</span>
              <span style={{ color: '#0f172a', fontWeight: 700 }}>{selectedSubject.name}</span>
            </div>

            <button
              onClick={() => router.push(`/subjective-all-mcqs-Practice-success?examId=${selectedExam.id}`)}
              style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(480px, 1fr))', gap: '22px' }}>
            {chaptersList.map(ch => {
              ...
              return (
                <div
                  key={ch.id}
                  onClick={() => router.push(`/subjective-all-mcqs-Practice-success?examId=${selectedExam.id}&subject=${selectedSubject.id}&chapterId=${ch.id}`)}
                  style={{
                    backgroundColor: '#ffffff',
                    borderRadius: '20px',
                    padding: '24px 22px',
    ...
    return (
      <main style={{ padding: '40px 0 80px', backgroundColor: '#f8fafc', minHeight: 'calc(100vh - 200px)' }}>
        <div className="container" style={{ maxWidth: '1300px', margin: '0 auto', padding: '0 20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '10px' }}>
            <div style={{ fontSize: '0.9rem', color: '#64748b' }}>
              <Link href="/subjective-all-mcqs-Practice-success" style={{ color: '#0284c7', textDecoration: 'none', fontWeight: 600 }}>সকল মডেল টেস্ট</Link>
              <span style={{ margin: '0 8px', color: '#cbd5e1' }}>/</span>
              <span style={{ color: '#0f172a', fontWeight: 700 }}>{selectedExam.title}</span>
            </div>

            <button
              onClick={() => router.push('/subjective-all-mcqs-Practice-success')}
              style={{
                backgroundColor: '#ffffff',
                color: '#334155',
                border: '1px solid #cbd5e1',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '26px' }}>
            {SUBJECTS_DATA.map(sub => {
              ...
              return (
                <div
                  key={sub.id}
                  onClick={() => router.push(`/subjective-all-mcqs-Practice-success?examId=${selectedExam.id}&subject=${sub.id}`)}
                  style={{
                    backgroundColor: '#ffffff',
                    borderRadius: '20px',
                    padding: '24px 22px',
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <button
                      onClick={() => router.push(`/subjective-all-mcqs-Practice-success?examId=${exam.id}`)}
                      style={{
...
export default function SubjectiveAllMcqsPracticeSuccessPage() {
  return (
    <Suspense fallback={
      <div style={{ textAlign: 'center', padding: '80px 20px', color: '#64748b' }}>
        বিষয়ভিত্তিক প্র্যাকটিস পেজ লোড হচ্ছে...
      </div>
    }>
      <ModelTestContent />
    </Suspense>
