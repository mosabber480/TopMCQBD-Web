...
const getJsonPath = () => path.resolve(process.cwd(), 'src', 'data', 'faq-data.json');

function getLocalFaqData() {
  try {
...
export async function GET() {
  try {
    ...
    const res = await fetch(`${cloudflareUrl}/api/faq-data`, {
      cache: 'no-store',
  ...
  return NextResponse.json(getLocalFaqData());
}
...
export async function POST(request) {
  try {
    ...
    try {
      const cloudflareUrl = getCloudflareBaseUrl();
      await fetch(`${cloudflareUrl}/api/faq-data`, {
        method: 'POST',
    ...
    return NextResponse.json({
      success: true,
      message: 'FAQ data saved and synced with Cloudflare D1!',
      data: body
    });
  } catch (err) {
    return NextResponse.json({ success: false, message: 'Failed to save FAQ data: ' + err.message }, { status: 500 });
  }
}
