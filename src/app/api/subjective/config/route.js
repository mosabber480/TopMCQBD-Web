...
const getConfigPath = () => {
  return path.resolve(process.cwd(), 'src', 'data', 'subjective-config.json');
};
...
const readConfig = () => {
  try {
    ...
    console.error('Error reading subjective-config.json:', err);
  }
...
const writeConfig = (data) => {
  try {
    ...
    console.error('Error writing subjective-config.json:', err);
    return false;
  }
};
...
export async function POST(request) {
  try {
    ...
    if (body.chapters) {
      current.chapters = body.chapters;
    }
    if (body.topics) {
    ...
    return NextResponse.json({
      success: true,
      message: 'Subjective configuration saved successfully',
      exams: current.exams,
