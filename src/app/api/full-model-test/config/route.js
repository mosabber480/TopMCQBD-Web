...
const getConfigPath = () => {
  return path.resolve(process.cwd(), 'src', 'data', 'full-model-config.json');
};
...
const readConfig = () => {
  try {
    ...
    console.error('Error reading full-model-config.json:', err);
  }
...
const writeConfig = (data) => {
  try {
    ...
    console.error('Error writing full-model-config.json:', err);
    return false;
  }
};
...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'Full Model Test configuration updated successfully!',
      exams: current.exams,
