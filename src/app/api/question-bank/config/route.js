...
const getConfigPath = () => {
  return path.resolve(process.cwd(), 'src', 'data', 'question-bank-config.json');
};
...
const readConfig = () => {
  try {
    ...
    console.error('Error reading question-bank-config.json:', err);
  }
...
const writeConfig = (data) => {
  try {
    ...
    console.error('Error writing question-bank-config.json:', err);
    return false;
  }
};
...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'Question bank configuration updated successfully!',
