...
// GET: Fetch status and all items from db-live-exam-test
export async function GET() {
  let client;
  const start = Date.now();
  try {
    const connection = await getLiveExamDb();
...
// POST: Add new text item to db-live-exam-test
export async function POST(req) {
  ...
  try {
    ...
    const insertResult = await db.collection('db-live-exam-test').insertOne(newDoc);

    return NextResponse.json(
      {
        success: true,
        message: 'ডাটা সফলভাবে db-live-exam-test কালেকশনে যুক্ত হয়েছে।',
        item: {
...
// PUT: Edit existing item in db-live-exam-test
export async function PUT(req) {
  ...
  try {
    ...
    const updateResult = await db.collection('db-live-exam-test').updateOne(filter, {
      $set: {
        text,
        updatedAt: new Date().toISOString(),
      },
...
// DELETE: Delete item from db-live-exam-test
export async function DELETE(req) {
  ...
  try {
    ...
    const filter = ObjectId.isValid(id) ? { _id: new ObjectId(id) } : { _id: id };
    const delResult = await db.collection('db-live-exam-test').deleteOne(filter);
