...
export default function DbD1Page() {
  return (
    <DbAuthGuard activeRoute="/db-connection-api/dbd1-api">
      <DbD1Content />
    </DbAuthGuard>
  );
}

function DbD1Content() {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const baseUrl = getApiBaseUrl();
      const res = await fetch(`${baseUrl}${DB_CONFIG.apiEndpoint}?id=${encodeURIComponent(id)}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSaveReorder = async () => {
    ...
    try {
      const baseUrl = getApiBaseUrl();
      const orderedIds = items.map((it) => it._id || it.id);
      const res = await fetch(`${baseUrl}${DB_CONFIG.apiEndpoint}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (!res.ok) throw new Error('ক্রম সেভ করতে সমস্যা হয়েছে।');

      setPendingReorder(null);
      showTopAlert('✅ নতুন ক্রম সফলভাবে আপডেট ও সেভ হয়েছে!', 'success');
      fetchDbData();
    } catch (err) {
      showTopAlert(`❌ ${err.message}`, 'danger');
    } finally {
  ...
  const collections = statusData?.collections || statusData?.keys || [
    'layout-config',
    'home-config',
    'sidebar-config',
    'policy-config',
  ...
  return (
    <main className="workers-api-container">
      <div className="workers-api-wrapper">
          ...
          <div className="top-box-bottom">
            <div className="copy-action-box">
              ...
              <div className="copy-field">
                <span className="copy-url-text" style={{ color: DB_CONFIG.badgeColor }}>https://{DB_CONFIG.hostName}{DB_CONFIG.apiEndpoint}</span>
                <button onClick={copyApiUrl} className="btn-copy" style={{ background: DB_CONFIG.badgeColor }} title="URL কপি করুন">
                ...
                {collections.length > 0 ? (
                  collections.map((col, idx) => (
                    <span
                      key={idx}
                      className="col-tag"
                      style={col === 'db-d1-test' ? { background: DB_CONFIG.badgeBg, color: DB_CONFIG.badgeColor, borderColor: DB_CONFIG.badgeBorder, fontWeight: '700' } : {}}
                    >
                      {col} {col === 'db-d1-test' && '★'}
                    </span>
                  ))
                ...
                {!isAddingData ? (
                  <div className="add-btn-wrapper">
                    <button
                      ...
                      style={{ background: DB_CONFIG.badgeColor }}
                    >
                  ...
                  <div className="add-rows-container">
                    <div className="add-rows-header">
                      <span className="add-rows-title">
                        <i className="fa-solid fa-layer-group" style={{ marginRight: '8px', color: DB_CONFIG.badgeColor }} />
                        নতুন ডাটা যোগ করুন:
                      </span>
                    </div>
        ...
        <DbNavBox activeRoute="/db-connection-api/dbd1-api" />
      </div>
