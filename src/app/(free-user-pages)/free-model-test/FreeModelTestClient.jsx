...
export default function FreeModelTestClient({ initialSearchParams }) {
  ...
  // Dynamic state loaded from config API
  const [exams, setExams] = useState(INITIAL_EXAMS);
  const [subjects, setSubjects] = useState(ALL_SUBJECTS_DATA);
  ...
  useEffect(() => {
    fetch('/api/free-model-test/config', { cache: 'no-store' })
      ...
      .catch(err => console.error('Free model test config fetch error:', err));
  }, []);
        ...
        <div className="container" style={{ maxWidth: '1300px', margin: '0 auto', padding: '0 20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '10px' }}>
            <div style={{ fontSize: '0.9rem', color: '#64748b' }}>
              <Link href="/free-model-test" style={{ color: '#0284c7', textDecoration: 'none', fontWeight: 600 }}>ফ্রি মডেল টেস্ট</Link>
              <span style={{ margin: '0 8px', color: '#cbd5e1' }}>/</span>
              <Link href={`/free-model-test?examId=${selectedExam.id}`} style={{ color: '#0284c7', textDecoration: 'none', fontWeight: 600 }}>
            ...
            <button
              onClick={() => router.push(`/free-model-test?examId=${selectedExam.id}`)}
              style={{
      ...
      <main style={{ padding: '36px 0 80px', backgroundColor: '#f8fafc', minHeight: 'calc(100vh - 200px)' }}>
        <div className="container" style={{ maxWidth: '1300px', margin: '0 auto', padding: '0 20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '14px' }}>
            ...
            <button
              onClick={() => router.push('/free-model-test')}
              style={{
                backgroundColor: '#ffffff',
                color: '#334155',
                border: '1px solid #cbd5e1',
          ...
          <div style={{ marginBottom: '32px' }}>
            ...
            <div style={{ marginTop: '18px' }}>
              ...
              >
                ...
                <span>{toBanglaNumber(subjects.length)} টি বিষয় অন্তর্ভুক্ত</span>
              </span>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '26px' }}>
            {subjects.map(sub => {
              ...
              return (
                <div
                  key={sub.id}
                  onClick={() => router.push(`/free-model-test?examId=${selectedExam.id}&subject=${sub.id}`)}
                  style={{
                    backgroundColor: '#ffffff',
                    borderRadius: '20px',
                    padding: '24px 22px',
              ...
              return (
                ...
                >
                  <div>
                    ...
                    {/* Title (Clickable) */}
                    <h2
                      onClick={() => router.push(`/free-model-test?examId=${exam.id}`)}
                      style={{
                        fontSize: '1.25rem',
                        fontWeight: 700,
                        marginBottom: '10px',
                    ...
                    >
                      <Link
                        href={`/free-model-test?examId=${exam.id}`}
                        style={{
                          color: '#0f172a',
                          textDecoration: 'none',
                          transition: 'color 0.2s ease'
