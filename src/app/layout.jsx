...
export async function generateMetadata() {
  try {
    await connectDB();
    const config = await LayoutConfig.findOne();
    const header = config?.header || {};

    return {
      title: header.seoTitle || header.siteTitle || 'TopMCQBD - সেরা অনলাইন কুইজ ও প্রস্তুতি প্ল্যাটফর্ম',
      ...
      icons: {
        icon: header.faviconUrl || '/images/favicon.ico',
        shortcut: header.faviconUrl || '/images/favicon.ico',
        apple: header.faviconUrl || '/images/favicon.png'
      }
    };
  } catch (e) {
    return {
...
async function getLayoutData() {
  try {
    ...
    console.warn('Could not load layout from DB during SSR, using defaults.');
  }
