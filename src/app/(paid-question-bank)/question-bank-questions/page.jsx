...
function QuestionsComponentInternal() {
  ...
  useEffect(() => {
    ...
    const qbUrl = categoryParam
      ? `/api/question-bank/questions?category=${encodeURIComponent(categoryParam)}`
    ...
    fetch(qbUrl)
      ...
      .then(async (data) => {
        ...
        // If question bank DB has no questions for this category yet, fallback to general API
        if (list.length === 0) {
          try {
      ...
      .catch((err) => {
        ...
        setError('ডাটাবেজ থেকে প্রশ্নব্যাংকের প্রশ্ন লোড করতে সমস্যা হয়েছে!');
        setLoading(false);
      });
  }, [categoryParam]);
  ...
  return (
    <div className="quiz-section-wrapper">
      ...
      >
        <h1>Question Bank Solutions & Practice</h1>
        <h2>{categoryParam ? formatCategoryDisplay(categoryParam) : 'প্রশ্নব্যাংক ও বিগত সালের প্রশ্ন সমাধান'}</h2>

        <div className="quiz-header-info-bar">
          <div className="quiz-exam-path">
            <Link href="/question-bank" style={{ textDecoration: 'none', color: 'inherit', display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
              <i className="fa-solid fa-book-bookmark" style={{ color: 'var(--primary, #007bff)' }}></i>
              <span style={{ textDecoration: 'underline' }}>প্রশ্নব্যাংক আর্কাইভ</span>
            </Link>
            <span style={{ margin: '0 6px', color: '#94a3b8' }}>/</span>
            <span>{categoryParam ? formatCategoryDisplay(categoryParam) : 'সকল প্রশ্নব্যাংক'}</span>
          </div>
