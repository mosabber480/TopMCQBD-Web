...
// Default Fallback Subjects Data
...
export default function FullModelTestClient({ initialSearchParams }) {
  ...
  useEffect(() => {
    fetch('/api/full-model-test/config')
      .then(res => res.json())
      .then(data => {
        if (data.success) {
  ...
  useEffect(() => {
    if (chapterId && examId && subjectId) {
      router.replace(`/full-model-test?examId=${examId}&subject=${subjectId}`);
    }
  ...
  // -------------------------------------------------------------
  // VIEW 3: MODEL TESTS UNDER SELECTED SUBJECT
  // (2 Boxes per line, with প্রশ্ন : ১০০ and সময় : ৬০ মিনিট)
  // -------------------------------------------------------------
  if (examId && subjectId && selectedSubject) {
    const MODEL_COUNT = selectedSubject.modelTestsCount || 20;
  ...
  // Shows Subject Cards with "২০ টি মডেল টেস্ট অন্তর্ভুক্ত"
  // -------------------------------------------------------------
  if (examId) {
    return (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '26px' }}>
            {subjects.map(sub => {
              ...
              return (
                <div
                  key={sub.id}
                  onClick={() => router.push(`/full-model-test?examId=${selectedExam.id}&subject=${sub.id}`)}
                  style={{
                    backgroundColor: '#ffffff',
                    borderRadius: '20px',
                    padding: '24px 22px',
                ...
                >
                  ...
                  <div>
                    ...
                    {/* Meta Specs Box inside Card showing Model Tests */}
                    <div
                      style={{
  ...
  // -------------------------------------------------------------
  // VIEW 1: ALL FULL MODEL TESTS LIST (Default / Home of full-model-test)
  // -------------------------------------------------------------
  return (
    <main style={{ padding: '40px 0 80px', backgroundColor: '#f8fafc', minHeight: 'calc(100vh - 200px)' }}>
      <div className="container" style={{ maxWidth: '1300px', margin: '0 auto', padding: '0 20px' }}>
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <button
                      onClick={() => router.push(`/full-model-test?examId=${exam.id}`)}
                      style={{
