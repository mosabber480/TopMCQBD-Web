...
export default function SubjectiveModelTestClient({ initialSearchParams }) {
  ...
  useEffect(() => {
    fetch('/api/subjective-model-test/config', { cache: 'no-store' })
      .then(res => res.json())
      .then(data => {
        if (data.success) {
      ...
      .catch(err => console.error('Failed to load subjective model test config:', err));
  }, []);
  ...
  if (examId && subjectId && chapterId && selectedSubject) {
    ...
    const topicsList = (topicsMap && topicsMap[subjectId] && (topicsMap[subjectId][currentChapter.id] || topicsMap[subjectId][chapterId])) || (CHAPTER_TOPICS[subjectId] && (CHAPTER_TOPICS[subjectId][currentChapter.id] || CHAPTER_TOPICS[subjectId][chapterId])) || [currentChapter.title];
...
{/* Sub-header / Breadcrumb Bar */}
        <div style={{ backgroundColor: '#ffffff', borderBottom: '1px solid #e2e8f0', padding: '12px 20px' }}>
          <div className="container" style={{ maxWidth: '1300px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
            ...
            <button
              onClick={() => router.push(`/subjective-model-test?examId=${selectedExam.id}`)}
              style={{
                backgroundColor: '#f1f5f9',
                color: '#334155',
                border: '1px solid #cbd5e1',
          ...
          <div className="container" style={{
            ...
            <div style={{ flex: '1 1 500px', zIndex: 1 }}>
              ...
              <p style={{
                ...
                {selectedExam.description || `${toBanglaNumber(subjectsData.length)} টি বিষয় — বিষয় বেছে নিন এবং অধ্যায়ভিত্তিক প্রস্তুতি ও পরীক্ষা শুরু করুন।`}
              </p>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '26px' }}>
            {subjectsData.map(sub => {
              ...
              return (
                <div
                  key={sub.id}
                  onClick={() => router.push(`/subjective-model-test?examId=${selectedExam.id}&subject=${sub.id}`)}
                  style={{
                    backgroundColor: '#ffffff',
                    borderRadius: '20px',
                    padding: '24px 22px',
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <button
                      onClick={() => router.push(`/subjective-model-test?examId=${exam.id}`)}
                      style={{
