...
function QuestionsComponentInternal() {
  ...
  useEffect(() => {
    ...
    const fmtUrl = categoryParam
      ? `/api/subjective-model-test/questions?category=${encodeURIComponent(categoryParam)}`
      : '/api/subjective-model-test/questions';
  ...
  return (
    <div className="quiz-section-wrapper">
      ...
      >
        <h1>বিষয়ভিত্তিক মডেল টেস্ট ও সমাধান</h1>
        <h2>{categoryParam ? formatCategoryDisplay(categoryParam) : 'বিষয়ভিত্তিক মডেল টেস্ট ও সমাধান'}</h2>

        <div className="quiz-header-info-bar">
          <div className="quiz-exam-path">
            <Link href="/subjective-model-test" style={{ textDecoration: 'none', color: 'inherit', display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
              <i className="fa-solid fa-graduation-cap" style={{ color: 'var(--primary, #007bff)' }}></i>
              <span style={{ textDecoration: 'underline' }}>বিষয়ভিত্তিক মডেল টেস্ট</span>
            </Link>
            <span style={{ margin: '0 6px', color: '#94a3b8' }}>/</span>
            <span>{categoryParam ? formatCategoryDisplay(categoryParam) : 'বিষয়ভিত্তিক মডেল টেস্ট'}</span>
          </div>
...
export default function SubjectiveModelTestQuestionsPage() {
  return (
