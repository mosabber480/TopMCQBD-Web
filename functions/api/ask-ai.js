 ...
 * Handles Ask AI requests on Cloudflare Edge with Gemini 1.5 Flash + Smart Fallback Engine
 */
...
5. সবসময় মার্জিত, উৎসাহব্যঞ্জক ও বন্ধুত্বপূর্ণ ভাষায় উত্তর দেবে।`;
    ...
    if (geminiApiKey) {
      try {
        ...
        if (geminiRes.ok) {
          ...
          if (candidateText) {
            return jsonResponse({ success: true, reply: candidateText });
          }
        }
      } catch (geminiErr) {
        console.warn('Cloudflare Gemini API error, falling back:', geminiErr);
      }
    }

    // Smart Educational Fallback
    let fallbackReply = '';
    if (question) {
    ...
    console.error('Cloudflare Ask AI Error:', err);
    return jsonResponse({ success: false, error: 'সার্ভার ত্রুটি হয়েছে। অনুগ্রহ করে আবার চেষ্টা করুন।' }, 500);
  }
}
